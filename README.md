...
Aplikasi berbasis **Command Line Interface (CLI)** sederhana untuk mengelola struktur organisasi kelas menggunakan konsep struktur data **Tree (Pohon)** di Python. Aplikasi ini mendukung penambahan, penghapusan, pencarian, pembaruan data anggota, serta penyimpanan otomatis ke file JSON.
...
* **Update Data Anggota (Baru!):** Mengubah nama atau jabatan anggota tanpa merusak struktur hierarki yang sudah ada.
...
Pastikan Anda sudah menginstal Python di komputer Anda (versi 3.x direkomendasikan). Aplikasi ini hanya menggunakan modul bawaan (`json`), jadi tidak perlu menginstal *library* pihak ketiga tambahan.
